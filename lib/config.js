      ...
      app.tab.query.active(function (tab) {
        if (tab && tab.url) {
          if (tab.url.indexOf("http") === 0) {
            ...
            var blacklist = config.blacklist.domains;
            var whitelist = config.whitelist.domains;
            var hostname = (new URL(tab.url)).hostname;
            var status = config.addon.state === "OFF" ? "disable" : (blacklist.indexOf(hostname) !== -1 ? "error" : (whitelist.indexOf(hostname) !== -1 ? "disable" : "enable"));
            /*  */
            for (var i = 0; i < 3; i++) {
            ...
            chrome.storage.local.get({"selected": {"name": "Default"}}, function (e) {
              ...
              var title = "Audio Equalizer" + (status === "error" ? " :: Error" : (status === "disable" ? " :: Disabled" : " :: " + e.selected.name));
              /*  */
